function opt=CaliAli_parameters_quest(varargin)
...
if isfield(opt, 'input_files')  % Prevent 'input_files' to propagate across modules
    opt = rmfield(opt, 'input_files'); % remove the "input files" from the opt 
    % to prevent propogation rather than create an empty field that would overwrrite 
    % manually setting / piping output from different functions 
end

if isfield(opt,'output_files')
    opt = rmfield(opt,'output_files'); % Prevent 'ouput_files' from propogating
end
